...
from ..krr.thin.util_k_mmd import (
    gauss, laplacian, sobolev,
    gaussian_M, laplacian_M,
    # singular, box
    box, epanechnikov
)
...
class KernelNadarayaWatsonBase(BaseEstimator):

    def __init__(self, kernel='epanechnikov', sigma=1, postprocess=None, M=None):
        assert kernel in [
            'gauss', 'laplace', 'sobolev', 
            'gauss_M', 'laplace_M', 
...
class KernelNadarayaWatsonRegressor(KernelNadarayaWatsonBase, RegressorMixin):
    pass
class KernelNadarayaWatsonClassifier(KernelNadarayaWatsonBase, ClassifierMixin):
    pass